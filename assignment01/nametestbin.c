...
void load_names_bsearch(FILE* fp, int year_index, tNames* names)
{
	...
	while (fgets(temp, sizeof(temp), fp) != NULL)//!=NUll
	{
		...
		sscanf(temp, "%s\t%c\t%d ", check.name, &check.sex, &check.freq[year_index]); //-> 구조체로 받는다, 이렇게 받고 구조체로 변경
		//qsort(names->data, names->len, sizeof(tName), compare); is it necessary?
		//tNames *address;
		//tName* address;
		address = bsearch(&check.name, names->data, initial, sizeof(tName), compare);
		...
		if (address != NULL)            //(address==&names->data[j])
		{
			...
			address->freq[year_index] = check.freq[year_index];  //check this part
			//printf(stderr, "%d\n", address->freq[year_index]);
		...
		else
		{
			...
			names->data[names->len].freq[year_index] = check.freq[year_index];  //this part check
			names->len++;
		}
